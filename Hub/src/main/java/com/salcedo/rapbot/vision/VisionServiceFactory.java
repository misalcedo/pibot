...
public abstract class VisionServiceFactory {
    public static VisionService http(final ActorSystem system, final Uri uri, final Path workingDirectory) {
        return new HttpVisionService(Http.get(system), ActorMaterializer.create(system), uri, workingDirectory);
    }

    public static VisionService url(final ActorSystem system, final Uri uri) {
        try {
    ...
    public static VisionService vlcj(MediaPlayer mediaPlayer, Path workingDirectory) {
